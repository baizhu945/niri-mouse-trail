...
- **漂移纠正**：靶状输入区域周期性从合成器获取绝对位置基准

这是在 Wayland 安全约束下的最优解——我们无法直接查询光标位置，因此将 evdev 追踪与合成器校准机会相结合。

### 靶状输入区域

初始 5 秒校准窗口过后，输入区域缩小为每个输出中心的**靶状图案**：
...
仅有十字形线条（10×10 中心方块 + 横竖双臂）接收指针事件，屏幕其余区域全部点击透传。

- **10×10 中心**（100 px²）：捕获跳屏时落在输出中心的光标
- **200×2 横臂**（400 px²）：捕获从中心水平方向离开的光标
- **2×200 竖臂**（400 px²）：捕获从中心垂直方向离开的光标
- **总面积**：约 900 px² — 不到 1440×900 表面的 0.07%

**为什么这样设计？** 当 niri 将光标跳转到另一屏幕时，光标落在输出中心。如果光标处于隐藏状态（niri 的 `hide-after-inactive-ms`），合成器可能延迟发送 `wl_pointer.enter` 事件。十字臂在光标从中心向任意方向移动时提供"二次捕获"机会。极小的面积确保日常点击几乎不可能被阻挡。

**为什么启动时需要 5 秒全表面？** 初始全表面窗口保证在拖尾首次启用时立即捕获光标位置，即使光标处于静止状态。捕获后由靶状区域处理后续跳屏。
