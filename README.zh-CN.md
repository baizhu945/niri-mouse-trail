...
> 程序启动或跳屏重启后，如果 `wl_pointer` 尚未捕获到光标位置，会暂时保留全屏输入区域等待捕获；等待期间也可能拦截点击。
>
...
> **本项目主要由 [GPT 5.6](https://openai.com/) 开发与调试，人工在 NixOS + niri 环境上进行测试验证。** 全部 C 代码、协议绑定、bug 修复均由 AI 辅助迭代生成。人类贡献者负责在双屏 HiDPI（1.77× / 1.60× 分数缩放）环境下实地测试，发现边界情况，并指导调试过程。
...
这将安装 `mouse-trail`、`mouse-trail-toggle`、`mouse-trail-ctl` 和 `mouse-trail-sync-theme`。
...
> **跳屏追踪是全自动的。** 程序通过 evdev 键盘设备监听跳屏快捷键（Super+Shift+Left/Right、Super+Shift+Ctrl+Left/Right 等，与鼠标是否隐藏无关），检测到后自动重启进程。新实例会一直保留全表面捕获区域，直到 `wl_pointer.enter` 或 `wl_pointer.motion` 提供绝对位置，不再超时回退到第一个屏幕；如果光标处于隐藏状态，等 compositor 后续产生指针事件后再完成校准。
...
- **启动校准**：一直保持全表面 `wl_pointer` 捕获，直到收到绝对位置
...
收到初始绝对位置后，输入区域缩小为每个输出中心的**空心环**（200×200 外框，196×196 内空，2px 线宽）：
...
仅 2px 粗的空心方框接收指针事件。环内和环外全部点击透传。
...
**跳屏（monitor switch）追踪。** niri 的 `focus-monitor-*` 会把光标瞬间传送到目标屏幕**正中央**——恰好落在环的空洞里，环无法直接捕获这次传送。因此程序通过 evdev 键盘设备监听跳屏快捷键（与鼠标是否隐藏无关），检测到后自动重启进程：新实例一直保持全表面捕获区域，直到 `wl_pointer.enter` 或 `wl_pointer.motion` 提供绝对位置，不再超时回退到第一个屏幕。重启由主线程在关闭旧的 Wayland 和输入资源后原地重新执行，避免多线程 fork 竞态和文件描述符泄漏。

**为什么要一直保持全表面？** Wayland 不提供全局光标位置查询。全表面输入区域可以让 `wl_pointer` 在光标静止时也报告绝对位置；捕获成功后再缩小为环形区域，恢复点击透传。
...
这发生在合成器启动时未发送 `wl_pointer.enter` 事件的情况（niri 上较常见）。拖尾会保留全输入区域并等待绝对位置，不再直接认定光标在主屏幕中心。跳屏后程序也会自动重启并持续等待捕获；如果光标仍处于隐藏状态，移动鼠标使 compositor 产生指针事件即可完成校准。
